...
fetch(urlApi)
    ...
    .then(data => {
        ...
        const tables = document.getElementById("first-table")
        const eventos = data.events
        console.log(eventos)
        cargarTabla1(eventos,tables)

        const tables2 = document.getElementById("up-table")
        const tables3 = document.getElementById("past-table")
...
function cargarTabla1(array, contendor) {
    ...
    trContenedor.innerHTML = `
        <td style="text-align:center">${mayorAttendance.name}: ${mayorAttendance.assistance/mayorAttendance.capacity*100}%</td>
        <td style="text-align:center">${menorAttendance.name}: ${menorAttendance.assistance/menorAttendance.capacity*100}%</td>
        <td style="text-align:center">${mayorCapacidad.name}: ${mayorCapacidad.capacity}</td>`
...
//  arreglo de categorias unicas
    ...
    for(categoria of categorias){
        let trContenedor = document.createElement('tr')
        trContenedor.innerHTML = `<td style="text-align:center">${categoria}</td>
        <td style="text-align:center">${calcularGanancias(array,categoria)}</td>
        <td style="text-align:center">${calcularAsistencia(array,categoria)}%</td>`
